...
const createTestServer = (opts = {}) =>
  createCert(opts.certificate).then(keys => {
    ...
    if (opts.bodyParser !== false) {
      server.use(
        bodyParser.json(Object.assign({ limit: '1mb', type: 'application/json' }, opts.bodyParser)),
      )
      server.use(
        bodyParser.text(Object.assign({ limit: '1mb', type: 'text/plain' }, opts.bodyParser)),
      )
      server.use(
        bodyParser.urlencoded(
          Object.assign(
            { limit: '1mb', type: 'application/x-www-form-urlencoded', extended: true },
            opts.bodyParser,
          ),
        ),
      )
      server.use(
        bodyParser.raw(
          Object.assign({ limit: '1mb', type: 'application/octet-stream' }, opts.bodyParser),
        ),
      )
    }
    ...
    server.listen = () =>
      Promise.all([
        pify(server.http.listen.bind(server.http))(opts.httpPort).then(() => {
          server.port = opts.httpPort || server.http.address().port
